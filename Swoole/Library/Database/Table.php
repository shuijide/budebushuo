...
class Table{
    ...
    {
        if (empty($insert)) throw new \Exception('写入的值不能为空',613);
...
//            $out_fields = $out_field_key =$this->tableColumns($this->table);
    ...
    //设置数据表字段变量 null | array
    protected function setTableFields()
    {
        $this->table_fields =$this->tableColumns($this->table);
    }
    ...
    protected function prepareSqlCondition($condition)
    {
        ...
        foreach ($condition as $con_key => $con_val) :
            ...
            if (!in_array($con_key,$this->table_fields)) {
                throw new \Exception($con_key.'不存在');
            }
